"""Detection and local persistence of camera/shot frame ranges."""
...
class CameraSegment:
    ...
    def create(cls, name: str, start: int, end: int, confidence: float = 1.0):
        if start < 0 or end < start:
            raise ValueError("Invalid camera segment range")
        return cls(uuid.uuid4().hex, name.strip() or "Câmera", start, end, "camera", confidence)
...
def detect_camera_segments(
    ...
    *,
    ...
    if total == 0:
        return []
    if total == 1:
        return [CameraSegment.create("Câmera 1", 0, 0)]
    ...
    for position, index in enumerate(sampled):
        if cancel_callback and cancel_callback():
            raise RuntimeError("Camera segment detection cancelled")
    ...
    for previous_index, index in zip(sampled, sampled[1:]):
        score = _signature_distance(signatures[previous_index], signatures[index])
        if score >= threshold:
            ...
            candidates.append((best_index, best_score))

    boundaries = [0]
    for index, score in sorted(candidates):
        if index - boundaries[-1] >= minimum_length and score >= threshold:
    ...
    for position, (start, stop) in enumerate(zip(boundaries, boundaries[1:]), 1):
        ...
        segments.append(
            CameraSegment.create(
                f"Câmera {position}", start, stop - 1, confidence=0.85
            )
        )
    if progress_callback:
...
def _frame_signature(path: str):
    ...
    thumbnail = cv2.resize(image, (64, 36), interpolation=cv2.INTER_AREA)
    histogram = cv2.calcHist([thumbnail], [0], None, [32], [0, 256])
    cv2.normalize(histogram, histogram)
    return thumbnail, histogram


def _signature_distance(first, second) -> float:
    first_image, first_histogram = first
    second_image, second_histogram = second
    ...
    return image_difference * 0.45 + histogram_difference * 0.55
